...
\section{Frontend}
    Das Frontend besteht aus zwei Webseiten, der Anwendung und einer externen Login-Seite.\\
    Die externe Login-Seite ist innerhalb des CDK-Stacks, im Cognito-Construct, definiert.
    Sie erlaubt es neue Nutzer zu erstellen und mittels eines dieser Nutzer sich anzumelden.\\
    Die Anwendung implementiert die Endpunkte des Backends und nutzt das Token der Loginseite, um Nutzer am ApiGateway-Endpunkt zu verifizieren.
    Die Umsetzung ist anhand der Graphik\ \ref{code:auth} ersichtlich.
    \begin{figure}[!h]
        \begin{lstlisting}[language=typescript, numbers=left, frame=single, basicstyle=\small\ttfamily]
...
root.render(
    <React.StrictMode>
        ...
        \caption{Geschützte Anwendung mit react OIDC}\label{code:auth}    
    \end{figure}\\    
\chapter{Building}
    ...
    Mittels des Tokens kann über das \textit{AWS CLI Tool} (download via npm) Zugriff auf AWS Ressourcen erlaubt werden.\\
    ...
    Beide Projekte haben readme-Dateien in ihrer Root, aus welchen die benötigten Befehle ersichtlich werden.
\chapter{Fazit}
    Bei dem Projekt handelt es sich um ein Übungsprojekt für erste Kontakte mit AWS.\@
    Deshalb hat es noch einige kleinere Logikprobleme.
    Vor allem ist ersichtlich, dass nicht alle Lambdafunktionen gleichermaßen ausgereift sind.\\
    Diese Probleme können in späteren Interationen über dieses Projekt beseitigt werden.
    Auch können neue Tests für die API eingeführt werden.
    Hier muss jedoch berücksichtigt werden, dass aufgrund der JWT-Tokenverfizierung ein Mocking eines Cognito-Userpools stattfinden muss.
    Ein solcher Mock stellt eventuelle Hindernisse dar, da dieser nicht ausreichend Dokumentiert ist.
    Auch sind die verwendeten Testbibliotheken teils noch in der Alpha-Phase, was zu möglichen Fehler führen kann, falls die Bibliothek geupdatet wird.
